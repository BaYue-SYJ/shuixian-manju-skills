...
把一本小说变成可拍 **AI 短剧** 的前期制作 Skill 三件套，专为 [WorkBuddy](https://www.workbuddy.cn) 设计。三个 Skill 各管一段、JSON 互相打通：
...
三个 Skill 全部 **零依赖、零 API key**，只用 Node.js 标准库；质量门靠代码而非模型自觉。出图为可选项，走 `codex` 内置图像生成。
